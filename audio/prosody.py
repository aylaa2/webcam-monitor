...
def analyze(samples: np.ndarray, sr: int = 16000) -> ProsodyFeatures:
    ...
    thr = max(0.02, 0.15 * float(np.percentile(rms, 95)))
    voiced = rms > thr
    ...
    # pause ratio: silent frames within the spoken span.
    idx = np.where(voiced)[0]
    if idx.size:
        span = voiced[idx[0]: idx[-1] + 1]
        pause_ratio = float(1.0 - span.mean()) if span.size else 0.0
    else:
        pause_ratio = 1.0
    ...
    return ProsodyFeatures(
        ...
        std_f0=std_f0,
        f0_range=f0_range,
        mean_energy=float(voiced_rms.mean()) if voiced_rms.size else 0.0,
